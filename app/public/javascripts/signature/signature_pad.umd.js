...
(function (global, factory) {
    ...
    class SignaturePad extends SignatureEventTarget {
        ...
        _getPointGroupOptions(group) {
            return {
                penColor:
                    group && "penColor" in group
                        ? group.penColor
                        : this.penColor,
                dotSize:
                    group && "dotSize" in group ? group.dotSize : this.dotSize,
                minWidth:
                    group && "minWidth" in group
                        ? group.minWidth
                        : this.minWidth,
                maxWidth:
                    group && "maxWidth" in group
                        ? group.maxWidth
                        : this.maxWidth,
        ...
        _strokeBegin(event) {
            this.dispatchEvent(
                new CustomEvent("beginStroke", { detail: event })
            );
        ...
        _strokeUpdate(event) {
            ...
            const pointGroupOptions =
                this._getPointGroupOptions(lastPointGroup);
            ...
            this._fromData(
                ...
                (curve, { penColor }) => {
                    ...
                    if (
                        ...
                        const attr =
                            `M ${curve.startPoint.x.toFixed(
                                3
                            )},${curve.startPoint.y.toFixed(3)} ` +
                            `C ${curve.control1.x.toFixed(
                                3
                            )},${curve.control1.y.toFixed(3)} ` +
                            `${curve.control2.x.toFixed(
                                3
                            )},${curve.control2.y.toFixed(3)} ` +
                            `${curve.endPoint.x.toFixed(
                                3
                ...
                (point, { penColor, dotSize, minWidth, maxWidth }) => {
                    ...
                    const size =
                        dotSize > 0 ? dotSize : (minWidth + maxWidth) / 2;
